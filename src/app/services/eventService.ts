...
/**
 * Upload image usando Cloud Function
 */
async function uploadImageViaFunction(
  imageData: string,
  fileName: string,
  eventSlug: string
): Promise<string> {
  try {
    const functionUrl = process.env.NEXT_PUBLIC_FIREBASE_FUNCTION_URL ||
      `https://us-central1-${process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID}.cloudfunctions.net/uploadEventImage`;
...
export async function createEventProfile(
  ...
  try {
    ...
    const imageFields = ["bgImage", "logoTop", "logoBottom", "frameImage", "buttonImage"];
    ...
    for (const field of imageFields) {
      ...
      const url = await uploadImageViaFunction(
        normalized,
        fileName,
        data.slug || "event"
      );
